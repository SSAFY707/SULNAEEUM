    ...
    return (
        <div className={''}>
            <div className={'flex pl-4  bg-zinc-100 items-center rounded-lg mt-[5px]'}>
                <HiMagnifyingGlass />
                <input onFocus={(e)=>{if(e.target.value) setFocus(true)}} onChange={search} className={' w-[200px] h-[40px] px-4 rounded-lg bg-zinc-100 outline-none '} placeholder='전통주 이름 검색' />
            </div>
            <div className='mt-[5px]'>

            </div>
            {focus && 
            <div className={'min-h-[150px] max-h-[250px] w-[280px] shadow-lg rounded bg-white absolute z-[200] overflow-y-scroll scroll'}>
                {result.length == 0 ?
