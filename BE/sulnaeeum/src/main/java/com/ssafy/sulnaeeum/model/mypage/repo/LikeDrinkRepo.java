...
public interface LikeDrinkRepo extends JpaRepository<LikeDrink, Long> {
    ...
    @Query(value = "select * from like_drink where drink_drink_id = ?1 and user_user_id = ?2", nativeQuery = true)
    public Optional<LikeDrink> findLikeInfo(Long drinkId, Long userId);
}
